...
/* private static String askForCells(Scanner scanner) {
        String cells;
        while (true) {
            System.out.println("Enter the cells");
            cells = scanner.nextLine();
            if (cells.length() != 9) {
                System.out.println("You must enter 9 cells. No spaces. Try again.");
                continue;
            }
            char [] checker = cells.toCharArray();
            boolean ok = true;
            for (char ch : checker) {
                if (!(ch == 'O' || ch == 'X' || ch == '_')) {
                    ok = false;
            ...
            if (!ok) {
                System.out.println("You can only enter 'X','O', or '_'. No spaces. Try again.");
                continue;
            }
            break;
        }
